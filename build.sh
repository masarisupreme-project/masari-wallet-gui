...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/masarisupreme-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    MONEROD_EXEC=masarisupremed.exe
fi
...
$QMAKE ../masarisupreme-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 
